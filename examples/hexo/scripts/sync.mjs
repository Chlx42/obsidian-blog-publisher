...
const CONTENT_DIR = join(process.cwd(), 'source/_posts')
...
function convertFrontmatter(obsidianFm) {
  // Hexo uses YAML frontmatter, similar to Hugo but with some differences
  const hexoFm = {
  ...
  if (obsidianFm.description) hexoFm.description = obsidianFm.description
  if (obsidianFm.heroImage) hexoFm.cover = obsidianFm.heroImage

  // Hexo doesn't use draft in frontmatter, it has a separate _drafts folder
  // We'll keep published posts in _posts and ignore draft field
...
function sync() {
  ...
  for (const file of files) {
    ...
    // Convert frontmatter to Hexo format
    const hexoFm = convertFrontmatter(frontmatter)

    // Write Hexo markdown
    const hexoContent = `---\n${Object.entries(hexoFm)
      .map(([key, value]) => {
    ...
    writeFileSync(outputPath, hexoContent)
...
function build() {
  log('Building Hexo site...')

  try {
    // Hexo uses its own generate command
    execSync('npx hexo generate', { stdio: isJson ? 'ignore' : 'inherit' })
    ...
    error('Hexo build failed')
  }
}

function publish() {
  log('Publishing with Hexo deploy...')

  try {
    // Hexo has built-in deploy command
    execSync('npx hexo deploy', { stdio: isJson ? 'ignore' : 'inherit' })

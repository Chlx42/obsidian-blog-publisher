...
<p align="center">
  <img src="docs/screenshot-panel.png" width="480"
       alt="博客面板：文章按状态分组（待发布 / 已上线 / 草稿 / 未发布）并带彩色状态点，已上线文章不再提供发布按钮，一键发布按钮显示待发布计数">
</p>
...
### 📍 文章留在原地
博客文章不必集中在文章文件夹里。文章目录之外的笔记只要 frontmatter 标记 `publish: true`，
同样会进入面板、预览和发布流程——笔记放在哪里由你决定，插件只读取，不挪动、不回写。

**不想手写 frontmatter？** 点一下状态栏的「＋ 加入博客」（或命令面板里的
「当前笔记加入博客」，默认 `Cmd/Ctrl+Shift+I`），缺失的字段自动补齐：
`publish: true` 进草稿箱、标题取文件名、日期取创建时间，description
直接提取正文第一段作草稿。已有内容一律不覆盖。
...
- **面板上的「发布」只是标记，唯一真正推送的入口是顶部「一键发布」。** 它依次执行同步 → 构建 → 发布命令；只有发布命令报告推送成功，插件才记快照，文章才变成「已上线」。推送失败或脚本报告没推送时，文章保持「待发布」，下次可以重推。没有待发布、也没有等待下线的隐藏文章时，「一键发布」按钮置灰，面板、ribbon 图标和命令面板都不会空跑一次发布。
...
| `title` / `description` / `publishDate` / `tags` | 你点「＋ 加入博客」或运行「当前笔记加入博客」命令 | 只补缺失键；description 取正文第一段作草稿，发布前自己润色 |

笔记正文永远不会被修改。slug 由同步脚本计算，写在站点仓库里，不回写 vault。

文章目录之外的笔记更进一步：**同步只读取、不回写**——想发布就点状态栏的
「＋ 加入博客」，或手动标记 `publish: true`。
...
| 文章文件夹 | vault 内的相对路径；目录外的笔记标记 `publish: true` 也会被识别 | 空 |
| 预览端口 | 开发服务器端口 | 4173 |

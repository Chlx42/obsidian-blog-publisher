...
#### 3. 添加文档文件
- [ ] `ARCHITECTURE.md` — 设计原理（manifest 如何工作、状态机、运行时抽象）
- [ ] `CONTRIBUTING.md` — 贡献指南（如何添加框架示例、如何报 bug）
- [ ] `CHANGELOG.md` — 版本历史
- [ ] Issues 模板（Bug Report、Feature Request、Framework Example）
...
### 4. 预览 URL 复制增强
- [ ] 预览运行时，每篇文章旁显示 `localhost:4173/blog/slug`
- [ ] 一键复制预览 URL
- [ ] 支持生产 URL 复制（从 siteUrl 拼接）
...
- [ ] 错误时显示「复制日志」按钮
- [ ] 失败后显示「重试」按钮
- [ ] 记录上次失败的操作，重启后提示「上次同步失败，是否重试？」

### 7. 乐观 UI 更新
- [ ] 点击「发布」「隐藏」「转草稿」时立即更新 UI
- [ ] 不等待文件写入完成（更流畅）
- [ ] 写入失败时回滚 UI 状态
